...
static void bfs_edge_data(const ECLgraph g, const int* const sp, data_type* const dist, const int* const wl1, const int wl1size, int* const wl2, int &wl2size, const int iter, int* const time, const int threadID, const int threadCount)
{
...
static double CPPbfs_edge(const int src, const ECLgraph& g, data_type* dist, const int* const sp, const int threadCount)
{
  ...
  do {
    ...
    for (int i = 0; i < threadCount; ++i) {
      threadHandles[i] = std::thread(bfs_edge_data, g, sp, dist, wl1, wl1size, wl2, std::ref(wl2size), iter, time, i, threadCount);
    }
    for (int i = 0; i < threadCount; ++i) {
