...
static void sssp_vertex_data(const ECLgraph g, data_type* const dist, data_type* const dist_n, const int* const wl1, const int wl1size, int* const wl2, int& wl2size, const int iter, const int threadID, const int threadCount)
{
...
static double CPUsssp_vertex(const int src, const ECLgraph& g, data_type* dist, const int threadCount)
{
  ...
  do {
    ...
    for (int i = 0; i < threadCount; ++i) {
      threadHandles[i] = std::thread(sssp_vertex_data, g, dist, dist_new, wl1, wl1size, wl2, std::ref(wl2size), iter, i, threadCount);
    }
    for (int i = 0; i < threadCount; ++i) {
