...
{
  return ((std::atomic<T>*)addr)->load(std::memory_order_relaxed);
}

template <typename T>
static inline void atomicWrite(T* const addr, const T val)
{
  ((std::atomic<T>*)addr)->store(val, std::memory_order_relaxed);
}
...
{
  T oldv = ((std::atomic<T>*)addr)->load();
  while (oldv != oldv+val && !(((std::atomic<T>*)addr)->compare_exchange_weak(oldv, oldv+val))) {}
  return oldv;
}
