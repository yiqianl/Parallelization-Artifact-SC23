...
if __name__ == "__main__":
    ...
    verify = args_val[1]
    input_path = inputs_folder + args_val[2]
    ...
    for i in range(len(cuda_path)):
        ...
        out_name = algorithms[i] + '_' + args_val[2] + '_cuda.out'
    ...
    for i in range(len(cpp_path)):
        ...
        out_name = algorithms[i] + '_' + args_val[2] + '_cpp.out'
        walk_code = os.walk(code_path)
        with open(out_name, 'w') as f:
    ...
    for i in range(len(omp_path)):
        ...
        out_name = algorithms[i] + '_' + args_val[2] + '_omp.out'
        walk_code = os.walk(code_path)
        with open(out_name, 'w') as f:
            for root, dircs, code_files in walk_code:
                for code_file in code_files:
